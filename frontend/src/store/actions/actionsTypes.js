export const REGISTER_USER_SUCCESS = 'REGISTER_USER_SUCCESS';
export const REGISTER_USER_FAILURE = 'REGISTER_USER_FAILURE';

export const LOGIN_USER_SUCCESS = 'LOGIN_USER_SUCCESS';
export const LOGIN_USER_FAILURE = 'LOGIN_USER_FAILURE';

export const LOGIN_USER_INIT = 'LOGIN_USER_INIT';

export const FETCH_REQUEST_SUCCESS = 'FETCH_REQUEST_SUCCESS';
export const FETCH_REQUEST_FAILURE = 'FETCH_REQUEST_FAILURE';

export const LOGOUT_USER = 'LOGOUT_USER';

export const GET_USER_SUCCESS = 'GET_USER_SUCCESS';
export const GET_USER_FAILURE = 'GET_USER_FAILURE';

export const GET_USERS_SUCCESS = 'GET_USERS_SUCCESS';
export const GET_USERS_FAILURE = 'GET_USERS_FAILURE';

export const ADD_USER_REQUEST = 'ADD_USER_REQUEST';
export const ADD_USER_FAILURE = 'ADD_USER_FAILURE';

export const DELETE_USER_SUCCESS = 'DELETE_USER_SUCCESS';
export const DELETE_USER_FAILURE = 'DELETE_USER_FAILURE';

export const CREATE_PRODUCT_INIT = 'CREATE_PRODUCT_INIT';
export const CREATE_PRODUCT_SUCCESS = 'CREATE_PRODUCT_SUCCESS';
export const CREATE_PRODUCT_ERROR = 'CREATE_PRODUCT_ERROR';

export const GET_PRODUCTS_SUCCESS = 'GET_PRODUCTS_SUCCESS';
export const GET_PRODUCTS_ERROR = 'GET_PRODUCTS_ERROR';

export const GET_PRODUCT_SUCCESS = 'GET_PRODUCT_SUCCESS';
export const GET_PRODUCT_FAILURE = 'GET_PRODUCT_FAILURE';

export const EDIT_PRODUCT_SUCCESS = 'EDIT_PRODUCT_SUCCESS';
export const EDIT_PRODUCT_FAILURE = 'EDIT_PRODUCT_FAILURE';

export const DELETE_PRODUCT_SUCCESS = 'DELETE_PRODUCT_SUCCESS';
export const DELETE_PRODUCT_FAILURE = 'DELETE_PRODUCT_FAILURE';

export const GET_HISTORIES_SUCCESS = 'GET_HISTORIES_SUCCESS';
export const GET_HISTORIES_ERROR = 'GET_HISTORIES_ERROR';

export const CREATE_REQUEST_INIT = 'CREATE_REQUEST_INIT';
export const CREATE_REQUEST_SUCCESS = 'CREATE_REQUEST_SUCCESS';
export const CREATE_REQUEST_ERROR = 'CREATE_REQUEST_ERROR';

export const GET_REQUEST_SUCCESS = 'GET_REQUEST_SUCCESS';
export const GET_REQUEST_ERROR = 'GET_REQUEST_ERROR';

export const GET_REQUEST_INFO_SUCCESS = 'GET_REQUEST_INFO_SUCCESS';
export const GET_REQUEST_INFO_FAILURE = 'GET_REQUEST_INFO_FAILURE';

export const GET_REQUESTS_SUCCESS = 'GET_REQUESTS_SUCCESS';
export const GET_REQUESTS_ERROR = 'GET_REQUESTS_ERROR';

export const PUT_REQUEST_SUCCESS = 'PUT_REQUEST_SUCCESS';
export const PUT_REQUEST_FAILURE = 'PUT_REQUEST_FAILURE';

export const CREATE_COORDINATE_SUCCESS = 'CREATE_COORDINATE_SUCCESS';

export const SET_LANGUAGE_SUCCESS = 'SET_LANGUAGE_SUCCESS';

export const ADD_GROUP_SUCCESS = 'ADD_GROUP_SUCCESS';
export const ADD_GROUP_FAILURE = 'ADD_GROUP_FAILURE';

export const GET_PERMISSIONS_SUCCESS = 'GET_PERMISSIONS_SUCCESS';
export const GET_GROUPS_SUCCESS = 'GET_GROUPS_SUCCESS';
export const GET_GROUPS_ERROR = 'GET_GROUPS_ERROR';

export const GET_GROUP_SUCCESS = 'GET_GROUP_SUCCESS';
export const GET_GROUP_ERROR = 'GET_GROUP_ERROR';

export const STATISTIC_INIT = 'STATISTIC_INIT';

export const GET_STATISTIC_SUCCESS = 'GET_PRODUCT_STATISTIC_SUCCESS';
export const GET_STATISTIC_ERROR = 'GET_PRODUCT_STATISTIC_ERROR';

export const GET_TRASH_SUCCESS = 'GET_TRASH_SUCCESS';
export const GET_TRASH_FAILURE = 'GET_TRASH_FAILURE';


export const LOADING_START = 'LOADING_START';
export const LOADING_STOP = 'LOADING_STOP';

export const ADD_USER_INIT = 'ADD_USER_INIT';
// WS actions

export const USERS_ONLINE = 'USERS_ONLINE'